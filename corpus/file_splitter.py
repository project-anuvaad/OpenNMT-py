...
for a,b in pairs:
    with open("corpus/original_data/IITB.en-hi-en.txt", "r") as text_file:
        with open("corpus/IITB_splitted_data/en-set-{0}-{1}".format(a,b),"w") as zh:
            for line in itertools.islice(text_file, a, b):
                 zh.write(line)
            # print(line)

    with open("corpus/original_data/IITB.en-hi-hi.txt", "r") as text_file:
        with open("corpus/IITB_splitted_data/hi-set-{0}-{1}".format(a,b),"w") as zh:
            for line in itertools.islice(text_file, a, b):
                zh.write(line)
            # print(line)
