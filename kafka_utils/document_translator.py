...
def testing_func2(translation_server):
    try:
        sentences =[]
        outfile = open("test2_o.txt", "w")
        with open("test2.txt", 'r', encoding='utf-8') as source:
            
            for line in source:
                sen = {"src":line,"id":61}
                sentences.append(sen)

        print("making call in func 2")
        out = translate_util.from_en(sentences, translation_server)  
        for translation in out['response_body']:
            res = translation['tgt']
            outfile.write(res)
            outfile.write('\n')
        outfile.close()      
        print("func2 resp:",out)
    except Exception as e:
        print("error2,",e)

def testing_func1(translation_server):
    try:
        sentences =[]
        outfile = open("test1_o.txt", "w")
        with open("test1.txt", 'r', encoding='utf-8') as source:
